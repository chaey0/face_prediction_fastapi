...
class Predict_FaceModel(tf.keras.models.Model):
    """
    Model for Predict_Face - removes mask from people faces using U-net neural network
    """
    ...
    @staticmethod
    @tf.function
    def psnr_loss(gt, y_pred, max_val=1.0):
        return tf.reduce_mean(tf.image.psnr(gt, y_pred, max_val=max_val))
    
    @staticmethod
    @tf.function
    def ssim_l2_loss(gt, y_pred, max_val=1.0, l2_weight=1.0):
        """
        Computes SSIM loss with L2 normalization
        @param gt: Ground truth image
        @param y_pred: Predicted image
        @param max_val: Maximal SSIM value
        @param l2_weight: Weight of L2 normalization
        @return: SSIM L2 loss
        """
        ssim_loss = 1 - tf.reduce_mean(tf.image.ssim(gt, y_pred, max_val=max_val))
        l2 = tf.nn.l2_loss(gt - y_pred)
        return ssim_loss + tf.cast(l2 * l2_weight, tf.float32)

    '''
    @staticmethod
    @tf.function
    def DISTS_loss(gt, y_pred):
        D=DISTS()
        # calculate DISTS between X, Y (a batch of RGB images, data range: 0~1)
        # X: (N,C,H,W) 
        # Y: (N,C,H,W) 
        dists_value = D(gt, y_pred)
        # set 'require_grad=True, batch_average=True' to get a scalar value as loss.
        dists_loss = D(gt, y_pred, require_grad=True, batch_average=True)
        dists_loss.backward()
        return   dists_loss
    '''
    ...
    def _download(url, output_dir):
        ...
        def _progress(count, block_size, total_size):
            print('\r>> Downloading %s %.1f%%' % (
                filename, float(count * block_size) / float(total_size) * 100.0), end="")
            sys.stdout.flush()
    ...
    @staticmethod
    @tf.function
    def LPIPS_loss(input0, input1, model='net-lin', net='alex', version=0.1):
        """
        ...
        for pb_fname in pb_fnames:
            if not os.path.isfile(os.path.join(cache_dir, pb_fname)):
                try:
                    Predict_FaceModel._download(os.path.join(Predict_FaceModel._URL, pb_fname), cache_dir)
    ...
    def load_model(model_path, configuration=None):
        ...
        @return: Predict_FaceModel
        """
        with CustomObjectScope({'ssim_loss': Predict_FaceModel.ssim_loss,
                            'ssim_l1_loss': Predict_FaceModel.ssim_l1_loss,
                            'psnr_loss': Predict_FaceModel.psnr_loss,
                            'ssim_l2_loss': Predict_FaceModel.ssim_l2_loss,
                            'LPIPS_loss':Predict_FaceModel.LPIPS_loss}):
            model = tf.keras.models.load_model(model_path)
        return Predict_FaceModel(model, configuration)
    ...
    def build_model(architecture: UNet, input_size: Tuple[int, int, int], filters: Optional[Tuple] = None,
        ...
        @return: Predict_FaceModel
        """
        return Predict_FaceModel(architecture.build_model(input_size, filters, kernels).get_model(), configuration)
    ...
    def train(self, epochs=20, batch_size=20, loss_function='mse', learning_rate=1e-4,
        ...
        train_dataset = Predict_FaceModel.tf_dataset(train_x, train_y, batch_size, predict_difference)
        valid_dataset = Predict_FaceModel.tf_dataset(valid_x, valid_y, batch_size, predict_difference, train=False)
        test_dataset = Predict_FaceModel.tf_dataset(test_x, test_y, batch_size, predict_difference, train=False)

        # select loss
        if loss_function == 'ssim_loss':
            loss = Predict_FaceModel.ssim_loss
        elif loss_function == 'ssim_l1_loss':
            loss = Predict_FaceModel.ssim_l1_loss
        # modification
        elif loss_function == 'psnr_loss':
            loss = Predict_FaceModel.psnr_loss
        # modification
        elif loss_function == 'ssim_l2_loss':
            loss = Predict_FaceModel.ssim_l2_loss
        # modification
        elif loss_function == 'LPIPS_loss':
            loss = Predict_FaceModel.LPIPS_loss
        else:
            loss = loss_function
        ...
        callbacks = [
            ModelCheckpoint(
                f'models/model_epochs-{epochs}_batch-{batch_size}_loss-{loss_function}_{Predict_FaceModel.get_datetime_string()}.h5'),
    ...
    def _test_results(self, test_x, test_y, predict_difference: bool):
        ...
        if self.configuration is None:
            result_dir = f'data/results/{Predict_FaceModel.get_datetime_string()}/'
        else:
            result_dir = os.path.join(self.configuration.get('test_results_dir'), Predict_FaceModel.get_datetime_string())
        os.makedirs(result_dir, exist_ok=True)

        for i, (x, y) in enumerate(zip(test_x, test_y)):
            x = Predict_FaceModel.read_image(x)
            y = Predict_FaceModel.read_image(y)
    ...
    def load_train_data(self, split=0.2):
        ...
        return Predict_FaceModel.load_data(os.path.join(train_dir, 'inputs'), os.path.join(train_dir, 'outputs'), split, limit)
    ...
    def load_test_data(self):
        ...
        return Predict_FaceModel.load_data(os.path.join(test_dir, 'inputs'), os.path.join(test_dir, 'outputs'), None, limit)
    ...
    def tf_parse(x, y):
        ...
        """
        def _parse(x, y):
            x = Predict_FaceModel.read_image(x.decode())
            y = Predict_FaceModel.read_image(y.decode())
            return x, y
    ...
    def tf_dataset(x, y, batch=8, predict_difference: bool = False, train: bool = True):
        ...
        dataset = dataset.map(Predict_FaceModel.tf_parse)
